...
/*!
    ...
    \return a parse_result struct.
        If the encoding is successful:
            ...
            - the scap_events vector will contain pointers to each encoded event, all located within
   scap_buf's memory
        ...
        If the buffer is too small to contain all encoded events:
            ...
            - the size field will be set to the total required size to fully translate the supplied
   gVisor event In case of any error:
            - the status field will be set to SCAP_FAILURE for parsing errors, SCAP_NOT_SUPPORTED

...
static parse_result parse_setresid(const char *proto, size_t proto_size, scap_sized_buffer scap_buf)
{
	...
	if(gvisor_evt.has_exit())
	{
		type = gvisor_evt.sysno() == 117 ? PPME_SYSCALL_SETRESUID_X : PPME_SYSCALL_SETRESGID_X;
	...
	{
		type = gvisor_evt.sysno() == 117 ? PPME_SYSCALL_SETRESUID_E : PPME_SYSCALL_SETRESGID_E;
...
static parse_result parse_setid(const char *proto, size_t proto_size, scap_sized_buffer scap_buf)
{
	...
	if(gvisor_evt.has_exit())
	{
		type = gvisor_evt.sysno() == 146 ? PPME_SYSCALL_SETUID_X : PPME_SYSCALL_SETGID_X;
	...
	{
		type = gvisor_evt.sysno() == 146 ? PPME_SYSCALL_SETUID_E : PPME_SYSCALL_SETGID_E;

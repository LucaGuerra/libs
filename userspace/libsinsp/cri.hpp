...
inline cri_interface<api>::cri_interface(const std::string &cri_path) {
	...
	auto deadline = std::chrono::system_clock::now() +
	                std::chrono::milliseconds(cri_settings::get_cri_timeout());
	...
	if(!status.ok()) {
		libsinsp_logger()->format(
		        ...
		        "cri: CRI runtime returned an error after version check at %s: %s",
		        cri_path.c_str(),
...
template<typename api>
inline grpc::Status cri_interface<api>::get_container_status_resp(
...
template<typename api>
inline grpc::Status cri_interface<api>::get_container_stats_resp(
        const std::string &container_id,
	...
	grpc::ClientContext context;
	auto deadline = std::chrono::system_clock::now() +
	                std::chrono::milliseconds(cri_settings::get_cri_size_timeout());
	context.set_deadline(deadline);
	return m_cri->ContainerStats(&context, req, &resp);
}

template<typename api>
grpc::Status cri_interface<api>::get_pod_sandbox_status_resp(
        const std::string &pod_sandbox_id,
...
inline std::string cri_interface<api>::get_container_image_id(const std::string &image_ref) {
	...
	case 0:
		libsinsp_logger()->format(sinsp_logger::SEV_WARNING,
		                          "Image ref %s not in list from CRI",
		                          image_ref.c_str());
		ASSERT(false);
		break;
	case 1: {
	...
	default:
		libsinsp_logger()->format(sinsp_logger::SEV_WARNING,
		                          "Image ref %s matches more than once in list from CRI",
		                          image_ref.c_str());
		ASSERT(false);
		break;
	}
...
inline bool walk_down_json(const json &root, const json **out, const std::string &key) {
...
inline bool walk_down_json(const json &root,
                           const json **out,
                           const std::string &key,
                           Args... args) {
	if(root.contains(key)) {
		return walk_down_json(root.at(key), out, args...);
	}
	return false;
}

inline bool set_numeric_32(const json &dict, const std::string &key, int32_t &val) {
	if(!dict.contains(key)) {
		return false;
	}
	const auto &json_val = dict.at(key);
	if(!json_val.is_number()) {
		return false;
	}
	val = json_val.get<int32_t>();
	return true;
}

inline bool set_numeric_64(const json &dict, const std::string &key, int64_t &val) {
	if(!dict.contains(key)) {
		return false;
	}
	const auto &json_val = dict.at(key);
	...
	val = json_val.get<int64_t>();
	return true;
}

template<typename api>
inline json cri_interface<api>::get_info_jvalue(
	...
	try {
		root = json::parse(info_it->second);
...
template<typename api>
inline bool cri_interface<api>::parse_cri_base(const typename api::ContainerStatus &status,
...
template<typename api>
inline bool cri_interface<api>::parse_cri_pod_sandbox_id_for_container(
        const json &root,
        sinsp_container_info &container) {
	if(root.is_null()) {
		return false;
	...
	if(root.contains("sandboxID") && root["sandboxID"].is_string()) {
		std::string pod_sandbox_id = root["sandboxID"].get<std::string>();
		container.m_pod_sandbox_id = pod_sandbox_id;
		// Add the pod sandbox id as a label to the container for backward compatibility
...
template<typename api>
inline bool cri_interface<api>::parse_cri_labels(const typename api::ContainerStatus &status,
                                                 sinsp_container_info &container) {
...
inline bool cri_interface<api>::parse_cri_image(const typename api::ContainerStatus &status,
	...
	if(image_name.empty() || strncmp(image_name.c_str(), "sha256", 6) == 0) {
		/* Retrieve image_name from annotations as backup when image name may (still) start with
		 * sha256 or otherwise was not successfully retrieved. Brute force try each schema we know
		 * of for containerd and cri-o container runtimes.
		 */
		if(!root.is_null()) {
			const json *jvalue;
			bool image_name_found = false;
			// walk_down_json(root, &image, "config", "image", "image")
			image_name_found = walk_down_json(root, &jvalue, "runtimeSpec", "annotations", "io.kubernetes.cri.image-name");
			if(!image_name_found) {
				image_name_found = walk_down_json(root, &jvalue, "runtimeSpec", "annotations", "io.kubernetes.cri-o.Image");
			}
			if(!image_name_found) {
				image_name_found = walk_down_json(root, &jvalue, "runtimeSpec", "annotations", "io.kubernetes.cri-o.ImageName");
			}
			if(image_name_found) {
				image_name = jvalue->get<std::string>();
...
template<typename api>
inline bool cri_interface<api>::parse_cri_json_imageid(const json &root,
                                                       sinsp_container_info &container) {
	if(root.is_null()) {
		return false;
	}
	...
	if(!walk_down_json(root, &image, "config", "image", "image") || !image->is_string()) {
		return false;
	}
...
template<typename api>
inline bool cri_interface<api>::parse_cri_mounts(const typename api::ContainerStatus &status,
                                                 sinsp_container_info &container) {
	for(const auto &mount : status.mounts()) {
		...
		container.m_mounts.emplace_back(mount.host_path(),
...
template<typename api>
inline bool cri_interface<api>::parse_cri_env(const json &root, sinsp_container_info &container) {
	if(root.is_null()) {
		return false;
	}

	const json *envs = nullptr;
	if(!walk_down_json(root, &envs, "config", "envs") || !envs->is_array()) {
		return false;
	}
...
template<typename api>
inline bool cri_interface<api>::parse_cri_ext_container_info(const json &root,
                                                             sinsp_container_info &container) {
	if(root.is_null()) {
		return false;
	}

	const json *linux = nullptr;
	if(!walk_down_json(root, &linux, "runtimeSpec", "linux") || !linux->is_object()) {
		return false;
	}
	...
	const json *cpu = nullptr;
	if(walk_down_json(*linux, &cpu, "resources", "cpu") && cpu->is_object()) {
	...
	const json *privileged = nullptr;

	// old containerd?
	if(walk_down_json(*linux, &privileged, "security_context", "privileged") &&
	   privileged->is_boolean()) {
		container.m_privileged = privileged->get<bool>();
		priv_found = true;
	}

	// containerd
	if(!priv_found &&
	   walk_down_json(root, &privileged, "config", "linux", "security_context", "privileged") &&
	   privileged->is_boolean()) {
		container.m_privileged = privileged->get<bool>();
		priv_found = true;
	}
	...
	// cri-o
	if(!priv_found && walk_down_json(root, &privileged, "privileged") && privileged->is_boolean()) {
		container.m_privileged = privileged->get<bool>();
		priv_found = true;
	}
...
template<typename api>
inline bool cri_interface<api>::parse_cri_user_info(const json &root,
                                                    sinsp_container_info &container) {
	if(root.is_null()) {
		return false;
	}

	const json *uid = nullptr;
	if(!walk_down_json(root, &uid, "runtimeSpec", "process", "user", "uid") ||
	   !uid->is_number_integer()) {
		return false;
	}

	container.m_container_user = std::to_string(uid->get<int>());
	return true;
}
...
template<typename api>
inline bool cri_interface<api>::parse_cri_base(const typename api::PodSandboxStatus &status,
                                               sinsp_container_info &container) {
	container.m_full_id = status.id();
	container.m_name = status.metadata().name();
	// This is in Nanoseconds(in CRI API). Need to convert it to seconds.
...
template<typename api>
inline bool cri_interface<api>::parse_cri_pod_sandbox_id_for_podsandbox(
        sinsp_container_info &container) {
...
// overloaded w/ PodSandboxStatus
template<typename api>
inline bool cri_interface<api>::parse_cri_labels(const typename api::PodSandboxStatus &status,
                                                 sinsp_container_info &container) {
	for(const auto &pair : status.labels()) {
		if(pair.second.length() <= sinsp_container_info::m_container_label_max_length) {
			container.m_labels[pair.first] = pair.second;
...
template<typename api>
inline bool cri_interface<api>::parse_cri_pod_sandbox_labels(
...
template<typename api>
inline bool cri_interface<api>::parse_cri_pod_sandbox_network(
        const typename api::PodSandboxStatus &status,
        const json &root,
        sinsp_container_info &container) {
	// Pod IP
	...
	const json *jvalue;
	bool cni_found = false;

	// Brute-force try all schemas for possible CNI results
	// pod info schema of CT_CONTAINERD runtime.
	cni_found = walk_down_json(root, &jvalue, "cniResult", "Interfaces");
	if(cni_found) {
		json cniresult_object;
		cniresult_object.update(*jvalue);
		// Remove members/fields not needed for incident response
		cniresult_object.erase("lo");
		for(auto it = cniresult_object.begin(); it != cniresult_object.end(); ++it) {
			const std::string &key = it.key();
			if(key.rfind("veth", 0) == 0) {  // Matches keys starting with "veth"
				cniresult_object.erase(it);
			} else {
				cniresult_object.at(key).erase("Mac");
				cniresult_object.at(key).erase("Sandbox");
			}
		}

		cniresult = cniresult_object.dump();
	} else {
		// pod info schema of CT_CRIO runtime.
		cni_found = walk_down_json(root, &jvalue, "runtimeSpec", "annotations", "io.kubernetes.cri-o.CNIResult");
		if(cni_found) {
			cniresult = jvalue->get<std::string>();
		}
	}

	// Cleanup: remove trailing newline, if any
	if(!cniresult.empty() && cniresult.back() == '\n') {
		cniresult.pop_back();
	}

	// Ensure result does not exceed the maximum length
	if(cniresult.size() > MAX_CNIRESULT_LENGTH) {
...
template<typename api>
inline bool cri_interface<api>::parse(const libsinsp::cgroup_limits::cgroup_limits_key &key,
                                      sinsp_container_info &container) {
	...
	libsinsp_logger()->format(sinsp_logger::SEV_DEBUG,
	                          "cri (%s): ContainerStatusResponse status error message: (%s)",
	                          container.m_id.c_str(),
	                          status.error_message().c_str());
	...
	if(!status.ok()) {
		...
		if(status.ok()) {
			/*
			 * We also want to ensure that the pod sandbox container stored in the container cache
			 * is fully filled out with available information as applicable. Most notably, the
			 * container's m_full_id and m_pod_sandbox_id will be the same, and the absence of
			 * container images can be attributed to the fact that they are not available for pod
			 * sandbox container processes. Another notable fact is that for pod sandbox containers
			 * container.m_lables and container.m_pod_sandbox_labels are also the same.
			 */
			...
			// `parse_cri_labels`: The pod sandbox container does not contain the namespace etc as
			// labels. To be consistent in the k8s filterchecks we retrieve the namespace from
	...
	if(!container_status_resp.has_status()) {
		libsinsp_logger()->format(sinsp_logger::SEV_DEBUG,
		                          "cri (%s): ContainerStatusResponse call no status, returning",
		                          container.m_id.c_str());
		ASSERT(false);
		return false;
	}
	...
	// In some cases (e.g. openshift), the cri-o response may not have an info property, which is
	// used to set the container user. In those cases, the container name stays at its default
	// "<NA>" value.
	...
	libsinsp_logger()->format(
	        sinsp_logger::SEV_DEBUG,
	        "cri (%s): after container parsing: repo=%s tag=%s image=%s digest=%s",
	        container.m_id.c_str(),
	        container.m_imagerepo.c_str(),
	        container.m_imagetag.c_str(),
	        container.m_image.c_str(),
	...
	if(cri_settings::get_cri_extra_queries()) {
		...
		/*
		 * The recent refactor makes full use of PodSandboxStatusResponse, removing the need to
		 * access pod sandbox containers in k8s filterchecks. Now, we also store the pod sandbox
		 * labels in the container. While this might seem redundant in cases where multiple
		 * containers exist in a pod, considering that the concurrent number of containers on a node
		 * is typically capped at 100-300 and many pods contain only 1-3 containers, it doesn't add
		 * significant overhead. Moreover, these extra lookups have always been performed for
		 * container ips in the past and therefore are no new additions.
		 */
		typename api::PodSandboxStatusResponse pod_sandbox_status_resp;

...
namespace std {
template<>
struct hash<sinsp_container_type> {
...
static inline bool is_docker_compatible(sinsp_container_type t) {
	return t == CT_DOCKER || t == CT_CRI || t == CT_CONTAINERD || t == CT_CRIO || t == CT_PODMAN;
}
...
public:
	 ...
	 */
	enum class state { STARTED = 0, SUCCESSFUL = 1, FAILED = 2 };

	state get_status() const { return m_state; }
	void set_status(state s) { m_state = s; }

	bool is_successful() const { return m_state == state::SUCCESSFUL; }
	 ...
	 */
	bool first_attempt() const { return m_retry == 0; }

	short retry_no() const { return m_retry; }

	void attempt_increment() { ++m_retry; }
...
public:
	...
	public:
		...
		container_mount_info(const nlohmann::json &source,
		                     const nlohmann::json &dest,
		                     const nlohmann::json &mode,
		                     const nlohmann::json &rw,
	...
	void set_lookup_status(sinsp_container_lookup::state s) { m_lookup.set_status(s); }
	sinsp_container_lookup::state get_lookup_status() const { return m_lookup.get_status(); }

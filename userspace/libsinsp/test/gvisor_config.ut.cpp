...
TEST(gvisor_config, generate_parse) {
	...
	ASSERT_TRUE(!root.is_discarded()) << "Could not parse configuration file contents.";
	...
	ASSERT_TRUE(root.contains(nlohmann::json::json_pointer("/trace_session/sinks/0/config/endpoint")));
	EXPECT_EQ(root[nlohmann::json::json_pointer("/trace_session/sinks/0/config/endpoint")], socket_path);
}
